 ...
 <div type="subTable" readonly="readonly" tablename="${subTable.tableName?lower_case}" ng-if="!permission.table.${toLower(subTable.tableName)}.hidden">
			...
			<#else>
				...
				<td ng-if="permission.table.${toLower(subTable.tableName)}.del || permission.table.${toLower(subTable.tableName)}.add">
				    <a class="am-btn am-btn-danger am-btn-xs" ng-click="removeRow('${toLower(subTable.tableName)}',$index)"
				    	ng-if="permission.table.${toLower(subTable.tableName)}.del">
			    		<i class="am-icon-remove"></i>删除</a>
					<a class="am-btn am-btn-primary am-btn-xs" ng-click="editRow('${toLower(subTable.tableName)}',$index)"
						ng-if="permission.table.${toLower(subTable.tableName)}.add">
  ...
  <div class="am-popup-inner">
    <div class="am-popup-hd">
      ...
      <span data-am-modal-close 
            class="am-close" onclick="javascript:$('#${toLower(subTable.tableName)}_editDialog').modal('close')">&times;</span>
    </div>
		...
		<#list fieldList as field>
			<#if field.isHidden == 0>
				 <div class="am-form-group" ng-if="permission.fields.${toLower(tableName)}.${field.fieldName}!='n'">
					...
					<div class="am-u-sm-9"> <@input field=field type=tableName/> </div>
				 </div>
			</#if>
		</#list>
 	</#assign>
