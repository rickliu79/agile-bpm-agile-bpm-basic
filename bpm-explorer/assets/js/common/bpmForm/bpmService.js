...
.directive('bpmButtons', ['bpmService','baseService', function(bpmService,baseService){
	return {
		...
		template:'<span  ng-repeat="button in buttonList">\
					...
					<div ng-if="button.alias==\'agree\'" 		buttonAlias="{{button.alias}}"   	ng-click="buttonClick(true,500,800)" 	class="btn btn-success fa fa-check-square-o">{{button.name}}</div>\
					<div ng-if="button.alias==\'oppose\'" 		buttonAlias="{{button.alias}}"   	ng-click="buttonClick(true,500,800)" 	class="btn btn-primary fa fa-close">反对</div>\
					<div ng-if="button.alias==\'reject\'" 		buttonAlias="{{button.alias}}"   	ng-click="buttonClick(false,500,800)" 	class="btn btn-danger fa fa-lastfm">驳回</div>\
